...
\section{Intoduction}
\paragraph{Confidential computing}
Confidential computing protects the data by running the various computations inside a secure environment, it adds a new layer of security, as before the data was only encrypted when stored, not when it's being used, but now it's encrypted even when it's being processed, protecting it from any unauthorized access.
It encrypts memory (including instructions and data) using keys managed by the AMD Secure Processor (ASP). Secure Encrypted Virtualization (SEV) encrypts the entire VM memory, so instructions and data remain encrypted while stored in RAM. Only the CPU decrypts them on-the-fly during execution.
...
%Unsure if it should be placed here
\paragraph{Side channel attacks}
A side channel attacks measures information indirectly produced by the physical system such as the time an operation takes to finish, the power consumed, eventual electromagnetic signals emitted or the patterns of how the memory is accessed to try and discover sensitive data.
\clearpage

%Contribution after the introduction
