...
\begin{figure}[htbp]
  ...
  {\textbf{Trampolines functionality}
   ...
   A small but important part that's omitted in the schema is the piece of code shown below:  
   }
  \label{fig:pbinc_trampoline_functionality}

\end{figure}

\begin{lstlisting}[language={[x86masm]Assembler}, caption={This part of the code allows the mapping of the memory to happen very slowly, so that it happens after the comparison with 0xdeadbeef and therefore it will jump to the call cascade.}, label={lst:slow-memory-map}]
