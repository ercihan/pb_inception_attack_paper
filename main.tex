...
These side-channel attacks exploit the timing differences caused by whether the data is cached or not.  
\begin{itemize}
  \item \textbf{Flush+Reload}
The attacker shares memory with victim (e.g., shared library page or C-bit=0 page in SEV), then the attacker flushes a cache line (via clflush). After that the victim accesses the memory (possibly transiently).
The attacker reloads and measures timing: if it's fast it means that the victim accessed it, if it's slow the victim did not.
  \item \textbf{Prime+Probe}
The attacker fills a cache set with own data, the victim executes, then the attacker measures which lines were evicted, revealing the victim’s accesses.
It also works without shared memory, but the precision is lower.  
  \item \textbf{Evict+Reload}
It works like Flush+Reload, but the eviction is done with cache-thrashing instead of using clflush.
\end{itemize}

\paragraph{Mitigations}
\begin{itemize}
  \item \textbf{Context switch (user$\rightarrow$user): IBPB and/or RSB filling} When switching from one user process to another, the kernel must prevent the new process from inheriting branch-prediction state from the previous one
  \begin{itemize}
    \item \textbf{IBPB (Indirect Branch Predictor Barrier)} A hardware instruction that clears indirect-branch prediction state.
    \item \textbf{RSB filling} The kernel overwrites the Return Stack Buffer with dummy return addresses to avoid the attacker influencing return-address speculation.
  \end{itemize}
  \item \textbf{User$\leftrightarrow$Kernel transitions} When switching between user mode and kernel mode, the kernel must avoid speculating on attacker-controlled targets
    \begin{itemize}
    \item \textbf{SMEP/SMAP} Hardware features that prevent the kernel from executing or accessing user memory—even speculatively.
    \item \textbf{(e)IBRS} Indirect Branch Restricted Speculation (enhanced IBRS) blocks the kernel from using user-mode branch history when predicting indirect branches.
    \item \textbf{BHI / RRSBA mitigations} These deal with Branch History Injection and Return/Retpoline Stack Buffer Array attacks. They involve clearing branch history on entry/exit and sometimes using IBPB again depending on policy.
  \end{itemize}
  \item \textbf{Return-path hardening for RETbleed/SRSO} They use return thunks to reduce dependence on RSB state.
    \begin{itemize}
    \item \textbf{Return thunks} A special code sequences that replace direct RET instructions with safer constructs that do not rely on RSB state. This prevents attackers from poisoning return predictions.
  \end{itemize}
\end{itemize}
