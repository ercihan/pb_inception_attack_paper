...
On AMD platforms, Secure Encrypted Virtualization (SEV) encrypts guest physical memory using per-VM keys managed by the AMD Secure Processor (ASP). Memory encryption is enforced outside the core; cache-line data are decrypted only transiently when accessed by the core and re-encrypted when written back. SEV-ES extends the protection boundary to guest CPU state (e.g. general-purpose registers during VM exits). \emph{SEV-SNP is a Zen~3+ feature and out of scope for Zen~1/Zen~2}; it adds integrity and replay protection for guest memory and metadata. For the Zen~1/Zen~2 systems we target, \textbf{only SEV/SEV-ES are available}. Whether a given page is encrypted is controlled by the SME \emph{C-bit}. Importantly, none of these mechanisms prevent in-core transient execution or eliminate microarchitectural side channels by themselves.
...
A common defense against cross-domain Spectre-BTI is the \emph{Indirect Branch Predictor Barrier} (IBPB), which software may execute on privilege or context transitions (e.g. on user$\rightarrow$kernel entry). The intended semantics are to sever cross-domain predictor influence. However, the efficacy of IBPB depends on microcode details and predictor composition. In particular, on AMD Zen~1/Zen~2, return-target predictions produced just \emph{before} the barrier can persist \emph{across} IBPB and influence the first \texttt{ret} executed afterwards when the entry path does not sanitize returns (e.g. Safe-RET/untrained-returns disabled). This creates a narrow post-IBPB speculation window in which the first return on entry can be transiently redirected to an attacker-chosen gadget, unless software additionally sanitizes return prediction state (e.g. RSB stuffing or untrained returns on entry).

\paragraph{Timing Side-channel attacks}
These side-channel attacks exploit timing differences caused by whether data are cached.  
\begin{itemize}
  \item \textbf{Flush+Reload}
The attacker shares memory with the victim (e.g. a shared library page or a C-bit=0 page under SEV/SEV-ES), flushes a cache line (e.g. via \texttt{clflush}), lets the victim execute (possibly transiently), then reloads and measures latency. A fast reload indicates the victim accessed the line.
  \item \textbf{Prime+Probe}
The attacker fills a cache set with its own lines, the victim executes, then the attacker measures which lines were evicted, revealing the victim’s accesses. This works without shared memory but typically has lower precision.
  \item \textbf{Evict+Reload}
Conceptually similar to Flush+Reload, but eviction is performed via cache-thrashing rather than \texttt{clflush}.
\end{itemize}

\paragraph{Mitigations}
\begin{itemize}
  \item \textbf{Context switch (user$\rightarrow$user): IBPB and/or return-predictor sanitization} When switching from one user process to another, the kernel must prevent the new process from inheriting prediction state from the previous one.
  \begin{itemize}
    \item \textbf{IBPB (Indirect Branch Predictor Barrier)} A barrier that clears (some) indirect-branch prediction state between security domains.
    \item \textbf{RSB filling / untrained returns (Safe-RET)} The kernel sanitizes return prediction state on transitions to reduce attacker influence on return speculation.
  \end{itemize}
  \item \textbf{User$\leftrightarrow$Kernel transitions} When switching between user mode and kernel mode, the kernel must avoid speculating on attacker-controlled targets.
    \begin{itemize}
    \item \textbf{SMEP/SMAP} Hardware features that prevent the kernel from executing or accessing user memory, reducing certain classes of speculative abuse of user mappings.
    \item \textbf{(e)IBRS} Indirect Branch Restricted Speculation (enhanced IBRS) restricts the influence of less-privileged branch history when predicting indirect branches in more-privileged code.
    \item \textbf{BHI / RRSBA mitigations} Mitigations for Branch History Injection and post-barrier return/retpoline predictor issues; these may include additional history clearing and/or IBPB depending on policy.
  \end{itemize}
  \item \textbf{Return-path hardening (RETbleed/SRSO-class)} Return thunks replace direct \texttt{ret} with safer constructs that do not rely on attacker-controlled return prediction state (e.g. Safe-RET/JMP2RET on AMD).
\end{itemize}

\paragraph{This work: PB-Inception}
We study and instantiate a post-IBPB speculative-execution primitive we term \emph{PB-Inception} on AMD Zen~1/Zen~2. Our constructive pathway trains return prediction immediately before a user$\rightarrow$kernel transition that executes IBPB-on-entry and then leverages the first post-IBPB \texttt{ret} in the kernel entry path to speculatively jump into a minimal data-cache gadget \emph{when return sanitization is disabled}. We further place the primitive in a Confidential Computing light setting (SEV, SEV-ES)\emph{without} violating the CC threat model: the gadget encodes one secret byte per entry into a deliberately shared, unencrypted buffer (\emph{C}-bit$=0$) that a host or helper can observe via \texttt{Flush+Reload}, while all protected data and architectural state remain covered by SEV and SEV-ES. This separation clarifies what CC light defends (confidentiality outside the core) and what it does not (in-core transient effects). Our contribution is implementation-focused: it packages prior post-barrier insights into an auditable, reproducible harness and control matrix.
...
In brief, we (i) systematize a post-IBPB return-hijack primitive on Zen~1/Zen~2; (ii) provide a constructive, reproducible PoC that trains in transient execution and hijacks the first post-IBPB return; (iii) show how to integrate the primitive with SEV and SEV-ES using a CC-compliant probe channel; and (iv) analyze where Linux entry paths and mitigation profiles defeat or permit the primitive. The detailed list appears in~\S\ref{sec:contribution}, with methodology in~\S\ref{sec:methodology} and a discussion of implications in~\S\ref{sec:results}.
...
\begin{longtable}{@{}%
  ...
  P{0.12\linewidth} %
@{}}
\caption{Related work overview. “Post‑IBPB” indicates whether the path relies on prediction state persisting across an IBPB boundary.}
...
\textbf{Spectre v1 / Spectre-BCB, (bounds check bypass) \cite{kocher2018spectre}} &
No &
Conditional-branch misprediction targeting the branch predictor/history (PHT) &
Same-process &
Bounds‑check bypass on mispredicted conditional; timer/eviction primitive &
Intel (Ivy Bridge, Haswell, Skylake), AMD Ryzen, ARM cores (Samsung/Qualcomm mobile chips) &
Serializing instructions (mfence, lfence or cpuid), LFENCE/speculation barriers &
Flush+Reload, Evict+Reload  &
Intra‑VM leakage; host observation requires shared C‑bit{=}0 mapping \\
\hline

\textbf{Spectre v2 / Spectre-BTI (branch target injection)
 \cite{kocher2018spectre}} &
No &
Indirect-branch poisoning targeting the Branch Target Buffer &
Cross-process &
Indirect‑branch poisoning; suitable training; timer &
Intel (Haswell, Skylake) &
IBRS, IBPB, STIBP, Retpoline &
Flush+Reload &
Same as above; host timing needs shared C‑bit{=}0 page \\
\hline

\textbf{SpectreRSB \cite{koruyeh2018spectrersb}} &
No &
Spectre-class targeting the Return Stack Buffer &
Same-process, Cross-process, Cross-thread &
RSB underflow/overfill; entry without RSB sanitization; early \texttt{ret} reachable &
Intel Core i7 (Haswell, Skylake, Skylake+), Intel Xeon E5-1620, any that uses an RSB should work &
RSB stuffing, Safe-RET, SMEP, KPTI (Kernel Page-Table Isolation) &
Flush+Reload, Prime+Probe &
Works intra‑guest; host view requires shared C‑bit{=}0 \\
\hline

\textbf{ret2spec \cite{maisuradze2018ret2spec}} &
No &
RSB-based speculative-execution &
cross-process &
RSB pollution; suitable return window; timer &
Intel x86 &
RSB stuffing, Safe-RET &
Flush+Reload, Prime+Probe &
As above (shared C‑bit{=}0 for host‑visible channel) \\
\hline

\textbf{INCEPTION \cite{trujillo2023inception}} &
No &
Training in Transient Execution (end-to-end exploit) targeting the Return Stack Buffer &
User $\rightarrow$ Kernel &
Pre‑entry return‑prediction poisoning; early \texttt{ret} after entry; IBPB‑only; timer &
AMD (Zen 1, Zen 2, Zen 3, Zen 4) &
Safe-RET, IBPB (Zen 1, Zen 2 only) &
Flush+Reload, Prime+Probe &
CC‑light: still leaks in‑core; host needs shared C‑bit{=}0 page \\
\hline

\textbf{RETBLEED \cite{wikner2022retbleed}} &
No &
Kernel-level speculative code execution targeting the Branch Target Buffer &
Unprivileged User Process $\rightarrow$ Kernel &
RSB underflow to BTB fallback (Intel); BTB precedence over RSB (AMD); deep call stacks (Intel); cross-privilege BTB poisoning &
AMD (Zen 1, Zen 2), Intel (Kaby Lake, Coffee Lake) &
Intel (eIBRS and IBRS), AMD (jmp2ret) &
Flush+Reload, Prime+Probe &
Bypasses Retpoline; leaks arbitrary kernel memory; intra-guest leakage; host view requires shared side-channel \\
\hline

\textbf{PB-RRSBA \cite{wikner2025breaking}} &
Yes &
Exploit IP-based predictions of the Restricted RSB Alternate (RRSBA) predictor &
Cross-process &
RSB empty-state; IP-based RRSBA predictor active; IBPB execution; same-thread attacker/victim co-location &
Intel (Golden Cove, Raptor Cove) &
Enabling the RRSBA\_DIS\_U chicken bit &
Flush+Reload &
Leaks from IBPB-protected suid processes; shared library mappings used for cross-process exfiltration \\
\hline

\textbf{PB-Inception \cite{wikner2025breaking}} &
Yes &
Variant of Inception attack targeting Return Stack Buffer predictions &
Cross-process &
IBPB-on-entry active; TTE/Phantom training pre-IBPB; RSB persistence post-IBPB (AMD Zen 1/2) &
AMD (Zen 1, Zen 2) &
RSB stuffing &
Secret-dependent cache side channel &
Bypasses kernel/hypervisor protection (VMExit); host observation requires shared C-bit=0 mapping \\
\hline

\textbf{PHANTOM \cite{wikner2023phantom}} &
No &
Confuse the CPU's branch prediction unit by using asymmetric combinations of training and victim instruction types &
Execute unprivileged code on top of a recent Linux kernel &
Speculation before instruction decode; cross-privilege BTB aliasing; physically contiguous huge pages for Prime+Probe &
AMD (Zen 1, Zen 2, Zen 3, Zen 4) &
SuppressBPOnNonBr (AMD MSR), AutoIBRS (AMD Zen 4), IBPB &
Flush+Reload, Prime+Probe &
Bypasses AutoIBRS/SuppressBPOnNonBr; enables transient fetch/decode leaks; host visibility needs shared memory \\
\hline
...
\begin{longtable}{
  P{3.2cm}  % Attack / Targeted structure (Post-IBPB?)
  P{3.0cm}  % Attacker model / boundary
  P{3.0cm}  % Preconditions   <-- this is the one that was overflowing
  P{3.8cm}  % Effective mitigations
  P{3.8cm}  % OS/μcode knobs
  P{2.2cm}  % Side-channel
  P{3.0cm}  % CC impact
}

\caption{Mitigation matrix by target structure, attacker boundary, preconditions, effective countermeasures, OS/,$\mu$code knobs, side channel, and CC (SEV/ES) impact.}
\label{tab:related-work-overview}\\

\toprule
\textbf{Attack / Targeted structure} &
...
\hline
\textbf{Generic mitigations \cite{kocher2018spectre,linux_srso_doc,linux_rsb_doc}} &
V1: same‑proc/ cross‑sandbox
V2: cross‑proc/ user$\rightarrow$kernel; Meltdown separate &
V1: bounds‑check bypass

V2: indirect‑ branch poisoning &
V1: hardening, SLH, LFENCE; Meltdown: KPTI

V2: IBRS/IBPB/Retpoline (RSB only for returns) &
...
\hline
\textbf{SRSO mitigation \cite{linux_srso_doc,amd_srso_whitepaper}} &
N/A &
N/A &
Safe‑RET / untrained‑return thunks at entry; optional RSB stuffing; policy on IBPB use &
\texttt{spec\_rstack\_overflow=
\{safe-ret,ibpb,off\}}, \texttt{spectre\_v2\_user}
\texttt{=prctl,ibpb}; CPUID per vendor &
N/A &
...
\subsubsection{Overview of the exploit flow}\label{sec:pb-inception-exploit-flow}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{pdfs/pbinc_gadget_functionality.drawio.pdf}%
  \caption[Gadget functionality]%
  {\textbf{Gadget functionality}
   Schema showing the functionality of the pbinc kernel gadget}
  \label{fig:pbinc_gadget_functionality}
\end{figure}
\end{landscape}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{pdfs/pbinc_trampoline_functionality.drawio.pdf}%
  \caption[Trampolines functionality]%
  {\textbf{Trampolines functionality}
   Schema showing the functionality of the return-address aliasing trampolines.  
   A small but important part that's omitted in the schema is the piece of code shown below ~\ref{lst:slow-memory-map}  
   }
  \label{fig:pbinc_trampoline_functionality}

\end{figure}
\end{landscape}

\begin{lstlisting}[language={[x86masm]Assembler}, caption={Slow predicate load: the load from \texttt{\_\_slow\_predicate\_ptr} delays branch resolution so the CPU follows a mispredicted backward conditional branch and transiently executes the call cascade.}, label={lst:slow-memory-map}]]
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{pdfs/pbinc_asm_functionality.drawio.pdf}%
  \caption[Assembly harness functionality]%
  {\textbf{Assembly harness functionality}
   Schema showing the functionality of the assembly harness used in the TTE call cascade}
  \label{fig:pbinc_asm_functionality}
\end{figure}
\end{landscape}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{pdfs/pbinc_driver_functionality.drawio.pdf}%
  \caption[Driver functionality]%
  {\textbf{Driver functionality}
   Schema showing the functionality of the driver that connects all the components}
  \label{fig:pbinc_driver_functionality}
\end{figure}
\end{landscape}
